#!/usr/bin/env python3
"""
Egyszerűsített Raktárkezelési Modul Teszt
...
def test_stock_issue_simple(inventory_service: InventoryService, test_data: dict):
    """Test stock issue (kiadás) functionality - simplified without work order"""
    
    print_header("KIADÁS TESZT - Egyszerűsített kiadás")
    ...
    print(f"📤 Egyszerűsített kiadás tesztelése")
    ...
    for part_idx, quantity in issue_data:
        ...
        movement = inventory_service.issue_stock(
            ...
            notes=f"Teszt kiadás - {part.name}"
        )
        movements.append(movement)
...
def test_double_entry_validation(inventory_service: InventoryService):
    ...
    print(f"   ✅ Egyenleg rendben: {'IGEN' if validation['is_balanced'] else 'NEM'}")
...
def main():
    ...
    print_header("GarageReg Egyszerűsített Raktárkezelési Rendszer Teszt")
    ...
    try:
        ...
        issue_movements = test_stock_issue_simple(inventory_service, test_data)
